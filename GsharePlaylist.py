	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.salemmax.gshare/(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.salemmax.gshare(/.+?/).+?\://", item["path"])
			...
			# elif any(service in item["path"] for service in ["4share.vn/f/", "fshare.vn/file"]):
				...
				item["path"] = "plugin://plugin.video.salemmax.search.youtube/%s/%s/" % (yt_route, yt_cid)
				...
				item["path"] = "plugin://plugin.video.salemmax.search.youtube/showimage/%s/" % urllib.quote_plus(item["path"])
			elif re.search("\.ts$", item["path"]):
				item["path"] = "plugin://plugin.video.f4mTester/?url=%s&streamtype=TSDOWNLOADER&use_proxy_for_chunks=True&name=%s" % (
					urllib.quote(item["path"]),
					urllib.quote_plus(item["label"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.salemmax.gshare" in item["path"]:
		...
		for tmp in tmps:
			try:
				...
				if resp.status == 404:
					...
					(resp, content) = http.request(
						"aHR0cHM6Ly9kb2NzLmdvb2dsZS5jb20vZm9ybXMvZC9lLzFGQUlwUUxTZUlTbzNjRklpQW5MZl9QS21kSmRyUXJsY0tsbHlNX2NCbWY4Nm1xVGhzcG8zNjd3L2Zvcm1SZXNwb25zZQ==".decode("base64"),
						"POST", headers = h,
						body=body
					)
					return ""
...
def GA(title="Home",page="/"):
	...
	try:
		...
		data      = {
			...
			'cid' : client_id,
			't'   : 'pageview',
			'dp'  : "SalemmaxPlayList%s" % page,
			'dt'  : "[SalemmaxPlayList] - %s" % title
		}
		http.request(
			ga_url, "POST",
